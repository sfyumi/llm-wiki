...
这是一个基于 LLM Wiki 模式的个人知识管理系统。所有知识以 Markdown 文件形式存储，Claude 负责自动化维护，人类负责审核确认。
...
公开网页一般直接 `WebFetch` 就够了。但有两类高价值源默认会被拦——遇到时按以下方法绕过，抓到原文后再落 `raw/notes/`。
...
- **Claude 的角色**：生成草稿、维护交叉引用、记录变更。所有写操作都是建议。
